...
def main():
    ...
    canvas.bind("<B1-Motion>", on_mouse_move)  # Привязываем к событию движения мыши
    canvas.bind("<ButtonRelease-1>", on_mouse_release)  # Отпускание кнопки мыши
    ...
    density_label.pack()
    density_slider = tk.Scale(frame_left, from_=0, to=100, orient="horizontal", command=on_rain_params_change)
    density_slider.pack()

    min_speed_label = tk.Label(frame_left, text="Min Speed")
    min_speed_label.pack()
    min_speed_slider = tk.Scale(frame_left, from_=0, to=10, orient="horizontal", command=on_rain_params_change)
    min_speed_slider.pack()

    max_speed_label = tk.Label(frame_left, text="Max Speed")
    max_speed_label.pack()
    max_speed_slider = tk.Scale(frame_left, from_=0, to=10, orient="horizontal", command=on_rain_params_change)
    max_speed_slider.pack()

    min_size_label = tk.Label(frame_left, text="Min Size")
    min_size_label.pack()
    min_size_slider = tk.Scale(frame_left, from_=5, to=20, orient="horizontal", command=on_rain_params_change)
    min_size_slider.pack()

    max_size_label = tk.Label(frame_left, text="Max Size")
    max_size_label.pack()
    max_size_slider = tk.Scale(frame_left, from_=10, to=40, orient="horizontal", command=on_rain_params_change)
