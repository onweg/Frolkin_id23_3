...
class Cloud:
    ...
    def is_clicked(self, x, y):
        # Проверка, попал ли курсор мыши в облако
...
def main():
    ...
    clouds = []
    selected_cloud = None  # Переменная для отслеживания выбранного облака
    ...
    def on_click(event):
        ...
        # Определяем, на каком облаке был клик
        selected_cloud = None
        for cloud in clouds:
    ...
    # Привязываем события
    canvas.bind("<Button-1>", on_click)  # Клик для выделения облака
    canvas.bind("<B1-Motion>", on_drag)  # Перетаскивание
